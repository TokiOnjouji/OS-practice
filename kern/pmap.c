...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	if(!(pgdir[PDX(va)]&PTE_P)) {
		...
		if(!tmp)
			return NULL;
		tmp->pp_ref++;
		pgdir[PDX(va)]=page2pa(tmp)|PTE_P|PTE_W|PTE_U;
	}
...
{
	pte_t* pte_tmp=pgdir_walk(pgdir,va,1);	
	if(!pte_tmp) return -E_NO_MEM;
	pp->pp_ref++;
	if(pte_tmp&&(*pte_tmp&PTE_P))
		page_remove(pgdir,va);
	*pte_tmp=page2pa(pp)|perm|PTE_P;
	return 0;
}
...
{
	pde_t* tmp=pgdir_walk(pgdir,va,0);
	if(!tmp) return NULL;
	if(!(*tmp&PTE_P)) return NULL;
	if(pte_store)
		*pte_store=tmp;
	return pa2page(*tmp);
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	perm|=PTE_P;
	void* i;
	for(i=ROUNDDOWN((void*)va,PGSIZE);i<ROUNDUP(va+len,PGSIZE);i+=PGSIZE) {
		int* p=(int*)pgdir_walk(env->env_pgdir,i,0);
		if((int)i>=ULIM||!p||((*p)&perm)!=perm) {
			if(i<va) user_mem_check_addr=(int)va;
			else user_mem_check_addr=(int)i;	
			return -E_FAULT;		
		}
	}
	return 0;
