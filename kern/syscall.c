	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	struct PageInfo *p;
	pte_t* pte;
	p = page_lookup(se->env_pgdir, srcva, &pte);
	if(!p) return -E_INVAL;
	if(!(perm&PTE_U) || !(perm&PTE_P) || (perm&~(PTE_U|PTE_P|PTE_W|PTE_AVAIL)))
		return -E_INVAL;
	if((perm&PTE_W) && !(*pte&PTE_W))
		return -E_INVAL;
	if(page_insert(de->env_pgdir, p, dstva, perm) < 0) {
		return -E_NO_MEM;
	}
	return 0;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if((uintptr_t)srcva < UTOP && (uintptr_t)srcva%PGSIZE != 0)
		return -E_INVAL;
	if((uintptr_t)srcva < UTOP && (!(perm&PTE_U) || !(perm&PTE_P) || (perm&~(PTE_U|PTE_P|PTE_W|PTE_AVAIL))))
		return -E_INVAL;
	if((uintptr_t)srcva < UTOP) {
		pte_t *pte;
