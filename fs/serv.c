...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	int len = file_read(o->o_file, ret, req->req_n, o->o_fd->fd_offset);
	if(len < 0) return len;
	o->o_fd->fd_offset += len;
	return len;
}
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	if ((r = openfile_lookup(envid, req->req_fileid, &o)) < 0)
		return r;
	int len = file_write(o->o_file, req->req_buf, req->req_n, o->o_fd->fd_offset);
	if(len < 0) return len;
	o->o_fd->fd_offset += len;
	return len;
}
...
serve_mmap(envid_t envid, struct Fsreq_mmap *req)
{
	...
	if(req->req_envid)
		envid = req->req_envid;
	return file_mmap(envid, req->req_va, req->req_length, o->o_file, req->req_offset, req->req_perm);
}
